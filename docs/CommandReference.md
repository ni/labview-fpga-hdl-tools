...
- gen-xdc regenerates the combined `objects/xdc/constraints.xdc` (the LabVIEW window netlist constraints **plus** your `add_custom_constraints` files) from the templates. After editing constraints, rerun `gen-xdc` — or `gen-vivado --update`/`--overwrite` (which auto-runs `gen-xdc`). A plain `gen-vivado` on an existing project does not reprocess; it asks for `--update`/`--overwrite`.
- gen-target regenerates the entire custom LabVIEW FPGA target plugin (a generated export under `objects/`). Rerun `gen-target` (then `install-target`) after changing anything the plugin depends on — the custom-I/O CSV, constraints, HDL, or `nihdlsettings.py`.
- install-target requires **all LabVIEW instances closed** before you run it, and LabVIEW must be **(re)started** afterward: LabVIEW FPGA only scans for target plugins at startup, so a running LabVIEW will not pick up a newly installed or updated target. (Installing under Program Files elevates via UAC.)
