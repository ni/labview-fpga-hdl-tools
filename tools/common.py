...
class FileConfiguration:
    ...
    use_gen_lv_window_files: bool  # Use files from the_window_folder to override what is in hdl_file_lists
...
def load_config(config_path=None):
    ...
    files.use_gen_lv_window_files = settings.get('UseGeneratedLVWindowFiles')
    ...
    if os.path.exists(window_constraints_path):
        with open(window_constraints_path, 'r') as f:
            ...
            if not period_clip_match or not from_to_match:
                print("Error: Could not find one or both marker sections in TheWindowConstraints.xdc")
                return
