...
def get_window_netlist(config):
    ...
    vivado_project_path = os.path.join(config.vivado_project_export_folder, "VivadoProject")
    print(f"Vivado project path: {vivado_project_path}")

    project_name = config.vivado_project_export_name
    ...
    if vivado_path:
        ...
        print(os.getcwd())
        #common.run_command(f'"{vivado_executable}" {project_name}.xpr -mode batch -source {get_netlist_tcl_path}', cwd=os.getcwd(), capture_output=False)
...
def copy_lv_generated_files(config):
    """
    Copy files that are generated by LV into TheWindow folder for use in the HDL vivado flow
    """
    protected_files_folder = os.path.join(config.vivado_project_export_folder, "NIProtectedFiles")

    # This list is hardcoded for now
    #
    # As we expand to future target support, we may need to make the list of LV FPGA generated files to copy
    # more dynamic based on the target
    files_to_copy = ["CodeGenerationResults.lvtxt", "PkgLvFpgaConst.vhd", "PkgCommIntConfiguration.vhd", "PkgDmaPortCommIfcRegs.vhd"]
...
def extract_lv_window_constraints(config):
    """
...
def main():
    ...
    get_window_netlist(config)
    copy_lv_generated_files(config)
    extract_lv_window_constraints(config)
