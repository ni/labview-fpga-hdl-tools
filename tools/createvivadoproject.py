...
def create_project(mode: ProjectMode, config):
    ...
    if vivado_path:
        ...
        if mode == ProjectMode.NEW:
            ...
            common.run_command(f'"{vivado_executable}" -mode batch -source {new_proj_path}', cwd=os.getcwd())
        elif mode == ProjectMode.UPDATE:
            # Update the existing project
            common.run_command(f'"{vivado_executable}" {project_name}.xpr -mode batch -source {update_proj_path}', cwd=os.getcwd())
        else:
